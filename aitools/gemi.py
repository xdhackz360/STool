...
def setup_gem_handler(app: Client):
    ...
    @app.on_message(filters.command(["gem"], prefixes=["/", "."]) & (filters.private | filters.group))
    async def gemi_handler(client: Client, message: Message):
        loading_message = None
        try:
            loading_message = await message.reply_text("**⚡️Generating response, please wait...⏳**")
    ...
    async def generate_from_image(client: Client, message: Message):
        ...
        processing_message = await message.reply_text("**⚡️Processing the image and generating response⏳**")
