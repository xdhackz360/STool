...
async def handle_fmail_command(client, message: Message):
    ...
    # Temporary message
    temp_msg = await message.reply_text("<b>⚡️ Fetching And Filtering Mails...⏳</b>", parse_mode=ParseMode.HTML)
...
async def handle_fpass_command(client, message: Message):
    ...
    # Temporary message
    temp_msg = await message.reply_text("<b>⚡️Filtering And Extracting Mail Pass...⏳</b>", parse_mode=ParseMode.HTML)
...
def setup_mail_handlers(app: Client):
    app.add_handler(handlers.MessageHandler(handle_fmail_command, filters.command(["fmail"], prefixes=["/", "."]) & (filters.private | filters.group)))
    app.add_handler(handlers.MessageHandler(handle_fpass_command, filters.command(["fpass"], prefixes=["/", "."]) & (filters.private | filters.group)))
