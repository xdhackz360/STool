...
def setup_education_handler(app: Client):
    @app.on_message(filters.command(["syn"], prefixes=["/", "."]) & (filters.private | filters.group))
    async def synonyms_handler(client: Client, message: Message):
        ...
        loading_message = await message.reply_text("**⚡️ Fetching Synonyms and Antonyms...⏳**", parse_mode=ParseMode.MARKDOWN)
    ...
    @app.on_message(filters.command(["tr"], prefixes=["/", "."]) & (filters.private | filters.group))
    async def translate_handler(client: Client, message: Message):
        ...
        loading_message = await message.reply_text(f"**⚡️ Translating Text Into {target_lang} language...⏳**", parse_mode=ParseMode.MARKDOWN)
